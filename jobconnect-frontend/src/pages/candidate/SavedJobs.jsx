import { useState, useEffect } from 'react'
import { Link } from 'react-router-dom'
import Navbar from '../../components/common/Navbar'
import Card from '../../components/common/Card'
import Badge from '../../components/common/Badge'
import Button from '../../components/common/Button'
import Loading from '../../components/common/Loading'
import savedJobApi from '../../services/api/savedJobApi'

export default function SavedJobs() {
  const [savedJobs, setSavedJobs] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetchSavedJobs()
  }, [])

  const fetchSavedJobs = async () => {
    try {
      const candidateId = localStorage.getItem('candidateId') || 1
      const response = await savedJobApi.getSavedJobs(candidateId, { page: 0, size: 20 })
      setSavedJobs(response.data.content || [])
    } catch (error) {
      console.error('Error fetching saved jobs:', error)
    } finally {
      setLoading(false)
    }
  }

  const handleUnsave = async (jobId) => {
    try {
      const candidateId = localStorage.getItem('candidateId') || 1
      await savedJobApi.unsaveJob(candidateId, jobId)
      setSavedJobs(savedJobs.filter(saved => saved.job.id !== jobId))
    } catch (error) {
      console.error('Error unsaving job:', error)
      alert('Failed to remove job from saved list')
    }
  }

  if (loading) return <Loading fullScreen text="Loading saved jobs..." />

  return (
    <div className="min-h-screen bg-gray-50">
      <Navbar />
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Saved Jobs</h1>
          <p className="text-lg text-gray-600">Jobs you've bookmarked for later</p>
        </div>

        {savedJobs.length === 0 ? (
          <Card className="text-center py-16">
            <div className="text-6xl mb-4">💾</div>
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No saved jobs yet</h3>
            <p className="text-gray-600 mb-6">Start saving jobs you're interested in to view them here</p>
            <Link to="/jobs">
              <Button variant="gradient">Browse Jobs</Button>
            </Link>
          </Card>
        ) : (
          <div className="space-y-4">
            {savedJobs.map((savedJob) => (
              <Card key={savedJob.id} hover>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-start gap-4">
                      <div className="w-16 h-16 bg-gradient-to-br from-primary-100 to-accent-100 rounded-lg flex items-center justify-center flex-shrink-0">
                        <span className="text-2xl font-bold text-primary-600">
                          {savedJob.job.company?.name?.charAt(0) || 'C'}
                        </span>
                      </div>
                      
                      <div className="flex-1">
                        <Link to={`/jobs/${savedJob.job.id}`} className="text-xl font-semibold text-gray-900 hover:text-primary-600 transition">
                          {savedJob.job.title}
                        </Link>
                        <p className="text-gray-600 mt-1">{savedJob.job.company?.name || 'Company'}</p>
                        
                        <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-gray-500">
                          <span className="flex items-center">
                            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                            </svg>
                            {savedJob.job.location || 'Not specified'}
                          </span>
                          <span className="flex items-center">
                            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                            {savedJob.job.jobType?.replace('_', ' ')}
                          </span>
                          {savedJob.job.salaryMin && savedJob.job.salaryMax && (
                            <span className="flex items-center">
                              <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
                              </svg>
                              ${savedJob.job.salaryMin.toLocaleString()} - ${savedJob.job.salaryMax.toLocaleString()}
                            </span>
                          )}
                        </div>

                        <div className="flex flex-wrap gap-2 mt-3">
                          <Badge variant="primary">{savedJob.job.jobType?.replace('_', ' ')}</Badge>
                          {savedJob.job.isRemote && <Badge variant="success">Remote</Badge>}
                          {savedJob.job.experienceLevel && (
                            <Badge variant="default">{savedJob.job.experienceLevel}</Badge>
                          )}
                        </div>

                        <p className="mt-3 text-gray-600 line-clamp-2">{savedJob.job.description}</p>
                        
                        <p className="mt-2 text-xs text-gray-500">
                          Saved {new Date(savedJob.savedAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  </div>

                  <div className="ml-4 flex flex-col gap-2">
                    <button
                      onClick={() => handleUnsave(savedJob.job.id)}
                      className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition"
                      title="Remove from saved"
                    >
                      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" />
                      </svg>
                    </button>
                    <Link to={`/jobs/${savedJob.job.id}`}>
                      <Button variant="primary" size="sm">View Details</Button>
                    </Link>
                  </div>
                </div>
              </Card>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}
